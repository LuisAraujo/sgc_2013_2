...
public class AdministradoresDAO {
	...
	protected int inserirAdministrador(Administrador adm) {
		...
		try {
			...
			sql = "INSERT INTO administradores(nome_admin,cpf_admin,usuarioAdmin, senha_admin) VALUES ("+"'"+adm.getNome()+"','"+adm.getCpf()+"','"+adm.getUsuario()+"','"+adm.getSenha()+"')";
	...
	protected int deletarAdministrador(Administrador adm) {
		...
		try {
			...
			sql = "delete from administradores where id_admin = '"+adm.getIdAdmin()+"'";
	...
	protected int buscarAdministradorPorID( Administrador adm) {
			...
			sql = "Select * from administradores where id_admin = '"+adm.getIdAdmin()+"'";
			...
			if (rs == null) {
				...
				try {
					...
					System.out.println("Não foi possivel encontrar este administrador");
				}
	...
	protected Administrador verificarLogin(String usuario, String senha) {
		...
		ResultSet rs = bdMysql.executarBuscaSQL(sql);
       if (rs == null) {
    	   //se a busca retornar null, o rs vai ser nul, e devolve o valor null
    	   return null;
		} else
			try {
				if (rs.next()) {
